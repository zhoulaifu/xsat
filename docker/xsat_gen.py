...
## In Z3, two expr_z3 has the same hash iff they are syntactically equivalent. Thus I suppose hash(a)=hash(b)=>a=b. The issue is that we rely on  this property, which can be a potential issue in the future. To refactore once I get some time.  
def var_name(expr_z3):
    #return "_t_"+str(expr_z3.hash())
    return "_t_"+str(expr_z3.get_id())


# cache is a dictionary from expr_z3 to its associated variable names
# result is the instruction to generate
# Update cache and result; return the segment corresponding to expr_z3. 
...
def _gen(expr_z3,symbolTable,cache,result):
    ...
    if _is_variable(expr_z3):
        ...
        ##populate the symbolic table, which maps symVar to symType
...
def gen(expr_z3):
   ...
   # foo_verify_dot_c=_get_template() %{"x_vars":x_vars,"x_types":x_types,"x_refs":x_refs,"x_expr":x_expr,"x_module":header_filename, "x_dim":x_dim,"x_body":result}

#    return foo_dot_c,foo_verify_dot_c, symbolTable
